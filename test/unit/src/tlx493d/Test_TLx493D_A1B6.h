...
TEST(TLx493D_A1B6, checkUnsupportedFunctionality)
{
    ...
    TEST_ASSERT_EQUAL_FLOAT( 0.0, dut.functions->getSensitivityScaleFactor(&dut) );
}
...
TEST(TLx493D_A1B6, setter_BitFields)
{
    ...
    for(uint8_t i = 0; i < GEN_1_BITFIELDS_COUNT; i++){
        ...
        if (bf->accessMode == TLx493D_WRITE_MODE_e){
            TLx493D_A1B6_setBitfield(&dut, i, (uint8_t) (pow(2, bf->numBits) - 1)); //set all bits of bitfield as HIGH 
            TEST_ASSERT_EQUAL((uint8_t) (pow(2, bf->numBits) - 1), (dut.regMap[bf->address + GEN_1_WRITE_REGISTERS_OFFSET] & bf->mask) >> bf->offset); // check if it matches the same value, we set above
...
TEST(TLx493D_A1B6, getter_BitFields)
{
    ...
    for(uint8_t i = 0; i < GEN_1_BITFIELDS_COUNT; i++){
        ...
        if (bf->accessMode == TLx493D_READ_MODE_e){
            ...
            dut.regMap[bf->address] = (uint8_t) ((dut.regMap[bf->address] & ((uint8_t) ~bf->mask)) | ((uint8_t) (pow(2, bf->numBits) - 1) << bf->offset));
            // then check against the same value as above
            TEST_ASSERT_EQUAL((uint8_t) (pow(2, bf->numBits) - 1), TLx493D_A1B6_returnBitfield(&dut, i));            
        }
    }
}
